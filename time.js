...
var showTime = function(){
    ...
    if (totle == 0) {
        ...
        if (SS <= 0){
            $(".result_page").show();
            $("#page_title").text($(".show_title").text());
        }
    }
    ;
    $(".time").html(SS);
	$(".game_time meter").val(SS);
};

var start1 = function(){
	//i = i + 0.6;
    // console.log(i);
	i = i + 100/((gameTime)*10);  //旋转的角度  90s 为 0.4  60s为0.6
	count = count + 1;
    myTimer = 100 - i;
    $(".bar").css("width", myTimer + "%");
    if(myTimer <= 35 ){
        $(".bar").attr("class","bar bar_mid");
    }
    if(myTimer <= 10 ){
        $(".bar").attr("class","bar bar_low");
    }
